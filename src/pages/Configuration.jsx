...
export default function ConfigurationPage() {
  const [soundEnabled, updateSoundEnabled] = useState(JSON.parse(localStorage.getItem("soundEnabled")) ?? true);
  const [trackWinnerState, updateTrackWinnerState] = useState(JSON.parse(localStorage.getItem("trackingState")) ?? false);
  const [lapSound, updateLapSound] = useState(localStorage.getItem("lapSoundRoute") ?? "");
  const [restartSound, updateRestartSound] = useState(localStorage.getItem("restartSoundRoute") ?? "");
  const [winnerSound, updateWinnerSound] = useState(localStorage.getItem("winnerSoundRoute") ?? "");

  function clear(callback) {
    const value = "";
    if (callback == "lapSoundRoute") {
      localStorage.setItem("lapSoundRoute", JSON.stringify(value));
      updateLapSound(value);
    }
    if (callback == "restartSoundRoute") {
      localStorage.setItem("restartSoundRoute", JSON.stringify(value));
      updateRestartSound(value);
    }
    if (callback == "winnerSoundRoute") {
      localStorage.setItem("winnerSoundRoute", JSON.stringify(value));
    ...
    <div className="max-w-[1200px] mx-auto">
      ...
      <article className="py-4 bg-mainbg rounded-lg m-4">
        <div className="flex flex-row justify-between items-center pr-8">
          <label className="inline-block ps-[0.15rem] indent-4 hover:cursor-pointer w-36 text-nowrap pr-4 select-none" htmlFor="trackWinnerState">
            Track Winner
          </label>
      ...
      <article className="py-4 bg-mainbg rounded-lg m-4">
        <div className="flex flex-row justify-between items-center pr-8">
          <label className="inline-block ps-[0.15rem] indent-4 hover:cursor-pointer w-36 text-nowrap pr-4 select-none" htmlFor="soundEnabled">
            Play Sounds
          </label>
          <div className="h-[1px] bg-slate-700 w-full"></div>
          <div className="pl-8">
      ...
      <article className="py-4 bg-mainbg rounded-lg m-4">
        <div className="flex flex-row">
          ...
          <input type="text" id="lapSound" className="mx-4 w-full bg-mainbg cursor-pointer" readOnly value={lapSound} onClick={() => handleFile("lapSoundRoute")} />
          <button onClick={() => clear("lapSoundRoute")} className="w-10 rounded-lg text-center hover:bg-red-400 mx-4">
            X
          </button>
        </div>
      </article>
      ...
      <article className="py-4 bg-mainbg rounded-lg m-4">
        <div className="flex flex-row">
          ...
          <input type="text" id="lapSound" className="mx-4 w-full bg-mainbg cursor-pointer" readOnly value={restartSound} onClick={() => handleFile("restartSoundRoute")} />
          <button onClick={() => clear("restartSoundRoute")} className="w-10 rounded-lg text-center hover:bg-red-400 mx-4">
            X
          </button>
        </div>
      </article>
      ...
      <article className="py-4 bg-mainbg rounded-lg m-4">
        <div className="flex flex-row">
          ...
          <input type="text" id="lapSound" className="mx-4 w-full bg-mainbg cursor-pointer" readOnly value={winnerSound} onClick={() => handleFile("winnerSoundRoute")} />
          <button onClick={() => clear("winnerSoundRoute")} className="w-10 rounded-lg text-center hover:bg-red-400 mx-4">
            X
          </button>
        </div>
      </article>
